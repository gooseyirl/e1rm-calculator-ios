...
struct MotivationalQuotes {
    ...
    static func nextQuote() -> String {
        let key = "quote_remaining"
        let stored = UserDefaults.standard.string(forKey: key) ?? ""

        var remaining: [Int]
        if stored.isEmpty {
            remaining = Array(0..<quotes.count)
        } else {
            remaining = stored.split(separator: ",").compactMap { Int($0.trimmingCharacters(in: .whitespaces)) }
            if remaining.isEmpty {
                remaining = Array(0..<quotes.count)
            }
        }

        guard let index = remaining.randomElement() else {
            return quotes[0]
        }
        remaining.removeAll { $0 == index }

        UserDefaults.standard.set(remaining.map(String.init).joined(separator: ","), forKey: key)
        return quotes[index]
    }
}
